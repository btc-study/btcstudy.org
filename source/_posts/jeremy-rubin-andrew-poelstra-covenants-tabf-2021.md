...
就 CAT 自身，你可以用来打碎签名。从代数上来说，不论 ECDSA 签名还是 Schnorr 签名，都有两个不同的部分，一个是数字的部分，我们通常称为 `s` ，另一部分是一个公钥，我们通常称为 `R`。 如果没有 OP_CAT，如果你尝试实现一些有趣的限制条款技巧，比如通过发现代数上的后门来把限制条款藏进协议中，那是很难做到的，因为你无法分割签名，因此无法将  `s` 跟  `R` 分割开来、专门限制某一样。   `s` 和 `R` 是完全不同的两种东西，而且 `R` 更难限制。
...
JR：有很多这样没有关联的事。如果我们有 CAT，我们也可以得到一个 CHECKSIGFROMSATCK 的变种。很多很多。我认为这也是 Andrew 前面的观点的一个很好的例子，也许我们会在未来意外地得到递归型限制条款，因为 CAT 这样不起眼的东西也能给比特币带来可怕的复杂性。力量都来自最让人意外的地方。
