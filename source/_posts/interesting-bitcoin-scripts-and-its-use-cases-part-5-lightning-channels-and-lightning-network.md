...
>
> 我们仍以上文的 State 1 为例。Alice 和 Bob 都要生成两对密钥。Alice 的两对密钥的公钥分别是 `S1A` 和 `P1A`；Bob 的分别是 `S1B` 和 `P1B`。他们都向对方请求公钥，也都向对方交出 `P 公钥`。然后，Alice 用如下方式求得一个新的公钥：
>
> ```
> RA1 = S1A * SHA256(S1A|P1B) + P1B * SHA256(P1B|S1A)
> ```
>
> 同理，Bob 也可以生成一个新的公钥：
>
> ```
> RB1 = S1B * SHA256(S1B|P1A) + P1A * SHA256(P1A|S1B)
> ```
>
> 这两个公钥会用在他们的承诺交易的输出中：
>
...
> ```
>
> 你发现了吗？这个 `pk(RA1)` 的行为跟前述哈希锁是非常相似的：（1）在 Bob 没有公布 P1B 的私钥之前，Alice 并不知道 RA1 的私钥，所以无法动用这个分支；（2）在 Bob 公布这个私钥之后，Alice 就知道了 RA1 的私钥，因此 Bob 不敢再发布这笔交易。而 Bob 只会在更新状态时揭晓 P1B 的私钥；Alice 亦然。
>
