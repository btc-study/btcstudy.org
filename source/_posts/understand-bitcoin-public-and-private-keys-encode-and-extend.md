...
扩展私钥是使用二进制私钥以及密语（passphrase）和派生路径（derivation path），根据你我都没必要知道的数学公式计算得出的。

请注意，增加密语会彻底改变下游扩展私钥的。修改派生路径也会改变下游数据。不要小看了你的软件钱包提供的默认派生路径，请务必把它写下来保存好。我会另外写一篇文章来详细介绍派生路径。
...
[点击此处，了解什么是派生路径](https://medium.com/@harshagoli/hd-wallets-explained-from-high-level-to-nuts-and-bolts-9a41545f5b0)。
