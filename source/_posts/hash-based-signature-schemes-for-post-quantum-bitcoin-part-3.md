...
这种构造给了我们 $k$ 个默克尔树根值 $\{R_1 ... R_k\}$，我们哈希这些树根，就能派生出 FORS 公钥 $P = H(R_1 ... R_k)$ 。

为了签名一条消息 $x$，我们使用一种随机化的子集挑选算法 $S_H(x)$ 对 $x$ 运行哈希计算，从而获得 $\vec{h} = \{h_1 ... h_k\}$，其中每一个数位 $h_i$ 都满足 $0 \le h_i \lt t$（就跟 HORST 一样）。但是，HORST 使用这些数位来检索 *单棵* 默克尔树上的叶子，而 FORS 则使用这些数位来检索 *各个* 默克尔树上的叶子 —— 每个数位对应一棵树 —— 从而选择要揭晓各个树上的哪个原像。这保证了 FORS 签名总是具有恒定的体积（与 HORS 和 HORS 不同）。
...
FORS 被设计成从一个小体积的种子生成秘密值原像，所以私钥的体积是固定的。给定一个秘密的 FORS 种子 $s$，我们可以用 $p_i = H(s, i)$ 来派生出第  $i$ 个原像。在派生出 $\{p_0 ... p_{tk-1}\}$ 个原像之后，我们可以将它们分组（每组有 $t$ 个原像），然后从中构造出 $k$  棵默克尔树。
...
签名人先采样出 $2^h$ 个随机值作为 OTS 私钥 $\{p_1 ... k_{2^h}\}$ ，然后计算它们的公钥  $\{P_1 ... P_{2^h}\}$。然后，将这些公钥的哈希值 $\{H(P_1) ... H(P_f{2^h})\}$ 作为叶子，构造出默克尔树，最终将其树根 $P$ 用作 MSS 公钥。
