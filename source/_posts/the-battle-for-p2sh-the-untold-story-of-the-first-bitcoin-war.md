...
这就是 OP_EVAL 提议的核心 —— 更容易实现的多签名钱包可以让用户即使弄丢了私钥备份也能找回比特币；服务商在检测到欺诈和偷盗之后，可以像银行一样发起警报；而且，更好的是，这些都可以在交易中实现，其外观和行为都像用户已经知道的那样。
...
自由和开源项目通常是由创始人领导的，而创始人反过来也必须跟他们的工作所依赖的贡献者保持一致。当然，在产生方向上的争议时，他们也拥有天然的权威来扮演他们的造物的决策者。
...
Andresen 被这个想法迷住了，他成为了第一个拥护者，在邮件组中发布了一份激情洋溢的请愿，号召贡献者一起行动。
...
这种担忧不是没有原因的 —— 在中本聪的实现中，多签名有巨大的缺点。最大的问题是，这些交易与比特币的标准地址格式不兼容，必须使用长得多的地址。
...
为了释放他看到的潜力，Andresen 开始鼓吹一个新的 “操作码”。所谓操作码，就是节点用来确定一笔交易是否有效的指令。
...
花费该笔资金所需满足的条件，只会在日后这笔资金 “从” 这个哈希值中花费的时候才会公开。因此，多签名的用户将在自己花费资金时为额外的交易体积付费，而且所需的额外数据对网络造成的负担也较小。
...
让事态更加复杂的是，开发者们假设为协议加入 OP_EVAL 在协作上会面临重大挑战。激活这样的变更必定会给区块链带来风险：区块链是对所有比特币交易的确定性记录，而这种形态是由整个网络对软件规则具有广泛的共识来保证的；引入这样的变更，可能会使网络分裂成互不兼容的部分。

这意味着，如果我们用的是所谓的 “硬分叉” 升级方法的话，当 OP_EVAL 要激活的时候，每一个用户都要切换成一个新的软件版本（或者说一个新的区块链网络）。
...
“哇，Gavin 说 OP_EVAL 不必造成分裂就可以出现吗，实在出乎我的意料”，Gregory Maxwell 立即对这个发现作出评论 [[9](https://web.archive.org/web/20131201200245/http://bitcoinstats.com/irc/bitcoin-dev/logs/2011/10/02)]，“值得干一杯（Bring out the [sic] champaign）！”
