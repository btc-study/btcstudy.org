...
先补充一个背景。规范（specifications）就应该没有 bug，我们系统它易于实现、不容易被误解。这使得规范的编写成了一件缓慢而且吃力不讨好的事。
...
换句话来说，伪代码是由你的直觉来定义的，python 代码是由 python 解释器来定义的。Python 没有形式化的语义。一段 python 代码到达是什么意思，是由解释它的 python 解释器决定的。但是，hac-spec 有结构化的操作性语义。每个操作码都有精确定义的功能。这些符号到底是什么意思并不重要，我们不要深究了，你不需要知道它们是什么意思。
...
另一方面，你可以证明这个规范在密码学证据上是否完整。这里，完整性意味着对有效的 BIP340 签名应用聚合算法讲总是能产生一个有效的减半聚合签名。这可以帮助减少 bug 。
