...
它就像函数式子一样，前面的 `thresh()` 表示使用门限条件，而这里的条件是 3 个公钥（以 `pk()` 括起）中的 `2` 个提供了签名。Policy 语言可以使用函数一共有 7 种 <sup><a href="#note3" id="jump-3">3</a></sup>，理解起来都很简单。
...
在企业财务中，为避免单点故障、不受约束的大权独揽，我们也可以使用多签名。在这里，多签名的用途除了避免单点故障，还有赋予权限、约束参与者的作用。给定（n - m + 1）个公钥是诚实的，资金就不会被任意挪用。
...
此外，Liquid Network 以及类似的应用往往不会只使用一个多签名设置，也就是说，同一笔资金实际上**有不止一套多签名的锁附加在上面**；这些多签名锁是相互独立的，而且都可以独自解锁资金。这是怎么做到的呢？我们文末揭晓。
...
它看起来毫不起眼，实际上非常强大。比如，我们可以在小孩子的压岁钱上设置两套多签名，一套是爸爸妈妈和小孩的 2-of-3 多签名，一套是爷爷奶奶和小孩的 2-of-3 多签名，从而小孩子只要攻破任何一个人，就可以花费自己的压岁钱。

同理，我们也可以形成设置多个托管方或多个仲裁方的合约。假设某个仲裁方的服务使双方都不满意，他们可以使用另一个仲裁方。
