...
>
> *编者注：原文作者在出版此文之后，撰写了一个新的版本，为 SuperScalar 增加/改变 了一些设计，见[此处](https://delvingbitcoin.org/t/superscalar-laddered-timeout-tree-structured-decker-wattenhofer-factories-with-pseudo-spilman-leaves/1242)。因此，为了解 SuperScaler ，本文应被视作一个过时的版本。*
...
- 必须无需变更区块链的共识。
  ...
  - 比特币区块链协议在实践中已经定型（ossified）。因为计划中每 4 年发生一次的增发减半会导致比特币汇率的突然升高，进而导致对比特币兴趣的突然增加，大量的新用户会从成为跟比特币的共识有利益相关的实体。当上一批用户被说服了要变更共识的时候，新的一批用户又进来了，他们也必须同意相同的共识变更。如果一项共识变更无法在短于减半周期的时间里获得实际共识，它就可能根本不会发生，从而导致实践中的协议定型（更小、更易于处理的变更可能依然能通过，但更复杂的变更，比如 “限制条款（covenant）”，可能永远不会获得共识）。
...
> 编者注：本文的[新版本](https://delvingbitcoin.org/t/superscalar-laddered-timeout-tree-structured-decker-wattenhofer-factories-with-pseudo-spilman-leaves/1242)在此段落前补充了一段说明：“举个例子，如果我们连锁 3 个 Decker-Wattenhofer 机制，那么我们需要 1 笔 ‘弹出’ 交易和 3 笔 ‘状态’ 交易；单向退出总计需要 4 笔交易”。这种情形可在下文的代码框中看出，只有最初一笔交易是弹出交易，后面都只是状态交易。所以，不如这么说：后续构造的弹出交易被上一个构造的状态交易省去了。在本文的后续段落中，作者依然使用 弹出交易-状态交易 交替出现的设计；但在新版本中，则完全使用仅用一笔弹出交易、后续都使用状态交易的设计；这构成了新老版本的一个重大区别。
...
当然，可能这个叶子中 `L` 自有的资金已经花完了，或者这个叶子已经到达了最终的状态（即，`nSequence = 0`）。这时候，这个 LSP 可以沿着树向上、找到更高层的状态交易、唤醒其他客户，以将其它叶子上的专有资金转移给 `A`，并重设该层以下的节点的 `nSequence`。这增加了需要同时在线的客户的数量，但依然不要求 *所有* 客户都在线。
