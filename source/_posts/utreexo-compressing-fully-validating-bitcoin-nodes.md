...
Utreexo [最初的论文](https://eprint.iacr.org/2019/611.pdf)出版于 2019 年 6 月 3 日，作者是 Tadge Dryja，是 [MIT Digital Currency Initiative](https://dci.mit.edu/) 的研究员，也是[闪电网络](https://lightning.network/)白皮书的联合作者。在 2020 年 6 月，Utreexo 的演示表面，这种轻量节点的思想已经成为[可用的代码](https://github.com/mit-dci/utreexo)。Utreexo 项目的其他贡献者包括：Calvin Kim、Janus Troelsen 和 Niklas Goggë。
...
假设一个用户要花费 7 号 UTXO。TA 需要证明这个 UTXO 存在：TA 提供 7 号叶子和 8 号叶子，h3 和 h5。验证的节点会使用这些数据创建出一棵单独的树，然后计算初根值。如果这个根值等于 h7，也即 Utreexo 节点存储的累加器根，则说明这个 UTXO #7 是存在的。
...
在实践中，有超过 6000 个 UTXO，它们会影城一个默克尔树的森林，在新区块产生时随着 UTXO 的 创建/花费 而 增加/删除 叶子，但这些都可以通过计算机非常高效地计算出来。
...
桥节点可以提供在一笔交易中花费的 UTXO 所对应的默克尔树叶子，为使用基于哈希的累加器的新型节点传输旧的网络中的常规交易。通过与全节点通信，桥节点可以存储完整的默克尔树森岭，使得 Utreexo 节点可以在收到新区块时更新自己的累加器根值。
