...
![img](../images/hash-based-signature-schemes-for-post-quantum-bitcoin-part-1/ndscape.jpg)
...
![image-20260302110958880](../images/hash-based-signature-schemes-for-post-quantum-bitcoin-part-1/quantum-adversary.svg)
...
- *带状态的（statefulness）*：签名人需要保留每一次签名之间的状态吗？
...
| **记号**                | 含义                                    |
| ----------------------- | --------------------------------------- |
| $\lbrace0, 1\rbrace ^n$ | 所有长度为 $n$ 的比特串的可能性集合     |
| $x \leftarrow S$        | 从集合 $S$ 中均匀随机采样出一个元素 $x$ |

令 $H(x) \rightarrow \lbrace0, 1\rbrace ^n$ 是一种密码学安全的哈希函数，会输出一个长度为 $n$ 的伪随机比特串（对于 SHA256 函数来说，`n = 256`）。下文所述的所有构造一般来说都基于这一种密码学原语。
...
> *[续篇见此处](https://www.btcstudy.org/2026/03/04/hash-based-signature-schemes-for-post-quantum-bitcoin-part-2/)：Lamport 签名、Winternitz 一次性签名（WOTS）、哈希以获得随机子集（HORS）*