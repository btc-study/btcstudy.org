...
$$ p_0 \leftarrow \lbrace 0, 1\rbrace ^n $$

$$ p_1 \leftarrow \lbrace 0, 1\rbrace ^n $$
...
$$ (p_0, p_1) : p_i \leftarrow \lbrace \lbrace 0, 1\rbrace  ^n \rbrace  ^m $$

其中，每一个 $p_i$ 都是一个包含了 $m$ 个随机原像 $\lbrace p_{(i, 1)}, p_{(i, 2)}, ... p_{(i, m)}\rbrace $ 的数组。
...
$$
\begin{align}
(P_0, P_1) : P_i &= \lbrace P_{(i, 1)}, P_{(i, 2)}, ... P_{(i, m)}\rbrace   \\\\
                 &= \lbrace H(p_{(i, 1)}), H(p_{(i, 2)}), ... H(p_{(i, m)})\rbrace 
\end{align}
$$

为了对一条消息（其比特值形式为 $\lbrace b_1, b_2, ... b_m\rbrace $）生成一个签名 $\sigma$ ，我们只需揭晓每一个比特值的对应原像：

$$ \sigma = \lbrace p_{(b_1, 1)}, p_{(b_2, 2)}, ... p_{(b_m,\ m)}\rbrace  $$
...
举个例子，如果 $m = 2$，而一个签名人为不同的两条消息 $\lbrace 0, 1\rbrace $ 和 $\lbrace 1, 0\rbrace $ 揭晓了两个签名，那就已经揭晓了其私钥 $(p_0, p_1)$ 中的所有（四个）原像。任何观察到这两个签名的人，都能用同一个公钥签名 $\lbrace 1, 1\rbrace $ 和 $\lbrace 0, 0\rbrace $，即使私钥的主人从未想过认可这些消息。
...
$$
\sigma = \lbrace (p_{(b_1, 1)}, P_{(1-b_0, 1)}) ... (p_{(b_m, m)}, P_{(1 - b_m, m)})\rbrace 
$$
...
$$ p = \lbrace p_1 ... p_\ell \rbrace \leftarrow  \lbrace \lbrace 0, 1 \rbrace ^n \rbrace ^\ell $$

为了计算 Winternitz 公钥 $P$，我们对每一个原像接连作 $w-1 = 2^t - 1$ 次哈希运算。即：

$$ P_i = H^{w-1}(p_i) $$

$$ P = \lbrace P_1 ... P_\ell \rbrace  $$

为了签名一条长度为 $m$ 的消息 $x$，我们以 $w$ 为基数，将 $x$ 转化为 $\ell_1$ 个数位（我将每一个数位都理解成一个长为 $t$ 比特的值）。

$$ x = \lbrace x_1 ... x_{\ell_1} \rbrace  : 0 \le x_i \lt w $$
...
$$ \vec{c} = \lbrace c_1 ... c_{\ell_2} \rbrace  : 0 \le c_i \lt w $$
...
\begin{align}
b &= x \parallel \vec{c}  \\\\
  &= \lbrace x_1 ... x_{\ell_1},\ c_1 ... c_{\ell_2}\rbrace 
\end{align}
$$
...
$$ \sigma = \lbrace \sigma_1 ... \sigma_\ell \rbrace  $$

对于验证者来说，给定消息原文 $x$ 和公钥 $P = \lbrace P_1 ... P_\ell \rbrace $ ，TA 必须先重新构造出带有校验和的消息 $b = x \parallel \vec{c}$ ，就像签名人一样。然后，验证者直接将签名中的哈希值继续哈希下去、得到最终哈希值，然后将最终哈希值与公钥 $P$ 对比。
...
如果我们不为这个长为两位的消息 $x$ 添加校验和，那么签名将直接是两个中间哈希值 $\lbrace H^3(p_1), H(p_2)\rbrace $。但是，任何知道这个签名的人，都能伪造在链条上处于 “靠后” 位置的消息 $x'$ 的签名，比如 $x' = \lbrace 3, 2\rbrace $ 或者 $x' = \lbrace 3, 3\rbrace $ 。
...
**紧凑公钥**：WOTS 非常适合通过哈希运算来压缩公钥的体积。一个 Winternitz 签名已经给了验证者重新构造出公钥 $P = \lbrace P_1 ... P_\ell\rbrace $ 的所有信息，所以我们可以用其哈希值来紧凑地表示一个 WOTS 公钥，代价知识验证者需要额外运行一次哈希运算。

$$P' = H(P) = H(\lbrace P_1 ... P_\ell \rbrace ) $$
...
WOTS+ 依赖一组随机化元素  $r = \lbrace r_1 ... r_{w-1} \rbrace $（每个元素的长度都是 $n$ 比特） ，以及一个 HMAC（基于哈希函数的消息认证码）密钥 $k$，全部都可以用哈希函数 $H$ 和一个种子派生出来，就像普通 WOTS 的 私钥 $p$ 一样。
...
$$ \sigma = \lbrace s, \sigma_1, ..., \sigma_\ell \rbrace  $$

紧凑的 WOTS+ 公钥 $P'$ 可以定义为一个简单的 $n$ 比特长的哈希值。

$$ P' = H(\lbrace c_k^{w-1}(p_1)\ ...\ c_k^{w-1}(p_\ell) \rbrace ) $$
...
为了生成一个 HORS 密钥，签名人需要先随机采集出 $t$ 个原像 $\lbrace p_0 ... p_{t-1}\rbrace $（其中 $t$ 是个巨大的数量）， 并以这些原像的哈希值作为自己的公钥 $P$ 。即：

$$ P_i = H(p_i) $$

$$ P = \lbrace P_0 ... P_{t-1} \rbrace  $$
...
HORS 的关键部分在于一种 *子集挑选* 算法 $S(x)$ 。给定一条消息 $0 \le x < \binom{t}{k}$ ，这个函数可以在集合 $\lbrace 0 ... t-1\rbrace $ 中返回一个由 $k$ 个随机元素组成的独特子集。也即：

$$ S(x) \rightarrow \lbrace h_1 ... h_k\rbrace  : 0 \le h_j \lt t $$

[HORS 论文](https://www.cs.bu.edu/~reyzin/papers/one-time-sigs.pdf)建议了两种[双射函数](https://en.wikipedia.org/wiki/Bijection)，它们满足了完美的一次性签名安全性所要求的 $S(x)$ 的子集挑选角色，确保了每一条消息都唯一对应于 $\lbrace 0...t-1\rbrace $ 中的一个包含 $k$ 个元素子集。

然而，这些双射函数并非单向函数，所以，如果一个签名人为同一个公钥提供了两个乃至更多个签名，观察者就能轻易伪造新的签名，办法是通过签名人已经揭晓的原像来反向确定自己能签名什么样的消息。

为了解决这个问题，HORS 转而使用一种单向的哈希函数 $S_H(x)$ 来挑选子集 —— 这正是它的名字 “哈希以获得随机子集” 的由来。这里讲 $S_H$ 的一种可能的实现方式。签名人计算输入消息的哈希值 $h = H(x)$，然后将 $h$ 解析为 $\log_2(t)$ 比特长的多个整数 $\vec{h} = \lbrace h_1 ... h_k\rbrace $ 。$H$ 的输出是一个 $n$ 比特长的比特串， 所以对每一个消息的哈希值，我们最多会得多 $\frac{n}{\log_2 t}$ 个整数， 但是，我们可以将 $H$ 的输出延长到任意长度（例如，通过串联它）。所以，这里没有硬性限制。
...
在计算出 $\vec{h} = S_H(x)$ 之后，HORS 签名人将整数们 $\vec{h} = \lbrace h_1 ... h_k\rbrace $ 映射为其私钥原像的一个子集  $\lbrace p_{h_1} ... p_{h_k}\rbrace $ 。这就形成了对消息 $x$ 的签名  $\sigma$ 。

$$ \sigma_i = p_{h_i} $$

$$
\begin{align}
\sigma &= \lbrace \sigma_1 ... \sigma_k \rbrace  \\\\
       &= \lbrace p_{h_1} ... p_{h_k} \rbrace
\end{align}
$$
...
> *[续篇见此处](https://www.btcstudy.org/2026/03/06/hash-based-signature-schemes-for-post-quantum-bitcoin-part-3/)：FORS、MSS 以及 SPHNICS+*