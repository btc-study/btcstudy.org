...
- 后续的另一次硬分叉，将 `脚本签名（scirptSig）` 和 `脚本公钥（scriptPubKey）` 的求值分割开来。这两者时比特币的价值转移系统的两个关键元素。脚本签名是比特币交易中的一些代码，包含了交易参与者的数字签名，以及验证交易合法性所需的其它信息。而脚本公钥则是交易的输出中的代码，指定了花费该输出中的资金所需满足的条件。脚本公钥和脚本签名两者一起构成了比特币的花费机制，保证了只有真正的比特币所有者能将资金转移到别的地方。中本聪发现的 bug 允许人们随意花费明明不属于自己的比特币；
...
- 9 月 7 日，另一次软分叉发生，添加了单个区块内的签名验证操作不得超过 2 万个的限制；如无次限制，可以创建一笔让节点崩溃的交易，因为交易可以要求验证极多的签名；
