...
但 PPLNS 在开发的时候还没有 Stratum v2 呢。更具体来说，PPLNS 不考虑矿工们行使自己的自由、使用自己的节点来构造自己的区块的可能性，而这正是 Stratum V2 鼓励的一种特性。这其中也包含了不去挖掘经济收益最高的区块的自用。这就是 PPLNS 的 “阿喀琉斯之踵” ，并且，随着交易手续费占据收益的比重越来越高，这个盲区还会越来越大。

因此，SLICE 将区块收益分为两部分，区块补贴（增发的比特币）和交易手续费。区块补贴使用经典的 PPLNS 方法来分发，完全基于回溯窗口（lookback winodw）内矿工贡献的哈希率数量。另一方面，交易手续费则根据所贡献的哈希率，以及与该哈希率相绑定的金融价值来分发。Stratum V2 中的 “作业生成器（Job Declarator，JD）” 会帮助矿工确定手续费的部分，这也是为什么 SLICE 也叫做 “PPLNS + JD” 。
