...
你也可以将这个方案延展呈一个两步骤的花费协议，就是第一步，需要用热钱包表示 “在 D 个区块中，允许花费至多 L 聪”；仅在此之后，你才能使用热钱包实际花费资金。在这种需求下，你要提供一个使用 COLD 作为内部公钥、带有两个脚本的 taproot 脚本公钥。第一个 “释放” 脚本指定：
...
- 使用 TLUV 来检查：
  ...
  - 默克尔路径修剪了一个元素
  - 当前的脚本依然包含在内
  - 默克尔路径增加了一个新的步骤，是下文的 “要么 H_LOCKED、要么 H_AVAILABLE” 脚本（取决于见证脚本信息提供的是 0 还是 1）
