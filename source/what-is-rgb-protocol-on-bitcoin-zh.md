...
[比特币上的 RGB 协议](https://rgb.info)是一个开源协议，可在比特币和闪电网络上原生发行。它给用着转让数字资产的机会，而无需使用侧链，无需对比特币基础层进行任何改动。所有资产数据保持私密且存储于链外，而比特币仅被用作承诺层，用于提供防篡改的锚定。
...
9. 谁在基于比特币上的 RGB 协议构建应用？
    
10. 比特币上的 RGB 协议与 Taproot Assets、Liquid 和以太坊相比如何？
    
11. 如何开始在比特币上的 RGB 协议上构建应用？
...
比特币区块链卓越来创建防篡改的所有权记录， 而任何人都可以在无需信任第三方的情况下就验证数据。但将这一模型扩展到比特币本身以外的数字资产，在历史上一直面临三个结构性问题。

早期在比特币上发行资产的方案，如彩色币（Colored Coins）、Counterparty 和 OmniLayer，将资产数据存储于链上。具体而言，第一代资产协议将资产元数据直接嵌入在比特币交易里面，通常是嵌入 OP_RETURN 输出中。每一笔资产转移都被永久写入链上，且对所有人可见。

![第一代代币协议示意图](../images/what-is-rgb-protocol-on-bitcoin-zh/img2-legacy-tokens-zh.png)

这种方式带来了三个相互叠加的问题：

区块链放大。 每笔资产转移都会增加数据，无论节点是否关心该资产，都必须永久存储这些数据。大规模应用后，运行全节点的成本将不断攀升，进而威胁去中心化。

隐私缺失。 每笔转移都永久可见。任何人都可以看到谁向谁发送了什么、数量多少、发生在何时。对于金融资产而言，这种情况消除了一项基本属性。

可扩展性差。 验证所有权需要从最初块开始扫描整条区块链，随着链的不断增长，这是一项日益昂贵的操作。

以太坊的 ERC-20 标准选择了另一条路：一个独立的全局状态系统，每个节点公开执行每一个合约。这一系统为了实现了大规模的可编程金融，不得不放弃牺牲隐私、比特币的安全模型和去中心化。
...
其核心理念是采用比特币区块链最擅长的特点，也就是说提供工作量证明的支撑和抗审查的结算。资产所有权记录、合约规则和转移历史均可在链外由直接有关的各方处理。区块链只需知道某件事已经发生，无需知道具体发生了什么。

因此，每个字节的数据不需要被所有人验证。事实上，在无需可信中介的情况下，比特币的全局共识必要来防止双花。但是对于其他类型的信息（资产合约规则、钱包余额、转移历史），没有理由通知整个网络。

我们可以用一份经公证的私人文件作为类比。公证人（比特币）确认某一特定事件发生于特定时间，且任何人都无法否认。但文件的内容完全保留在相关各方之间。
...
比特币上的 RGB 协议通过[一次性密封条](https://docs.rgb.info/distributed-computing-concepts/single-use-seals)防止双花：每个 RGB 资产都绑定到一个比特币 UTXO，花费该 UTXO 即允许转移，从而使双花不可能发生， 否则等同于双花比特币。
